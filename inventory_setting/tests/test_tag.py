...
class TagTestCase(TestCase):
    ...
    def test_url_resolves_to_tag_page(self):
        found = resolve('/inventory/1/1/settings/tags')
        self.assertEqual(found.func, tag.tags_settings_page)

    def test_tag_settings_page_returns_correct_html(self):
        client = Client()
        client.login(
            username=TEST_USER_USERNAME,
            password=TEST_USER_PASSWORD
        )
        response = client.post('/inventory/1/1/settings/tags')
